...
void PostController::initRoutes(WebServer &webServer)
{
    ...
    webServer.webApp.get("/posts", [this, &webServer]APPLICATION(req, res){
        ...
        webServer.database.mariadb.query(
            ...
            "WHERE post.thread__id=" + threadId,
            [this, &webServer, &res, threadId] (const MYSQL_ROW row) -> void {
                if (row != nullptr) {
                    // Collect all posts
                    posts = posts +
                    ...
                    res.send(
                        ...
                        "<div class=\"main\">"
                            ...
                            "<form action=\"/createPost\" method=\"post\" enctype=\"multipart/form-data\">"
                                "<input type=\"hidden\" name=\"users__id\" value=\"" + currentUserId + "\" />"
                                "<input type=\"hidden\" name=\"thread__id\" value=\"" + threadId + "\" />"
                                "<textarea name=\"text\"></textarea><br>"
                                "<button " + buttonDisabledLoggedOff + ">Add post</button>"
                            "</form>"
                            "<div class=\"comments\">"
                                + posts +
                            "</div>"
    ...
    // Create new post
    webServer.webApp.post("/createPost", [this, &webServer]APPLICATION(req, res) {
        ...
        webServer.database.mariadb.exec(
            "INSERT INTO `post`(`text`, `timestamp`, `users__id`,`thread__id`) VALUES('" + text + "', CURRENT_TIMESTAMP(), '" + userId + "', '" + threadId +"')",
            [&res, threadId](int num) -> void {
