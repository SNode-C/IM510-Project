...
void UserController::initRoutes(WebServer &webServer)
{
    ...
    webServer.webApp.post("/register", [this, &webServer]APPLICATION(req, res) {
        ...
        webServer.database.mariadb.query(
            "SELECT * FROM `users`",
            [this, userName, email, password, &webServer, &res](const MYSQL_ROW row) -> void {   // Gets called for each row found.
                if (row != nullptr) {
                    if (row[1] == userName) {
                        this->success = 0;
                    }
    ...
    webServer.webApp.post("/login", [this, &webServer]APPLICATION(req, res) {
        ...
        if (isLogin(body)) {
            if (userName.size() != 0 && password.size() != 0) {
                webServer.database.mariadb.query(
                    "SELECT * FROM `users`",
                    [this, userName, password, &res, &webServer](const MYSQL_ROW row) -> void {   // Gets called for each row found.
                        if (row != nullptr) {
...
std::string UserController::parseRequest(std::string &str, const std::string &keyword)
{
    int line = 0;
    int goalLine = 0;
    int nCount = 0;
