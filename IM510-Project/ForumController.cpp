...
void ForumController::initRoutes(WebServer &webServer) {
  ...
  webServer.webApp.get("/addExamples", [&webServer] APPLICATION(req, res) {
    ...
    webServer.database.mariadb.exec(
        "INSERT INTO `thread`(`id`, `name`, `timestamp`, `users__id`, "
        "`topic__id`) VALUES " // remove ids?
    ...
    webServer.database.mariadb.exec(
        "INSERT INTO `post`(`id`, `text`, `timestamp`, `users__id`, "
        "`thread__id`) VALUES "
        ...
        "(12, 'PStet clita kasd gubergren, no sea takimata sanctus est Lorem "
        "ipsum dolor sit "
        "amet.', NOW(), '4', '1'),"
        "(13, 'LOL', NOW(), '5', '3'),"
        ...
        "(19, 'Duis autem vel eum iriure dolor in hendrerit in vulputate', "
        "NOW(), '6', '5'),"
        "(20, 'Psssssss', NOW(), '2', '6');",
  ...
  webServer.webApp.get("/deleteAll", [&webServer] APPLICATION(req, res) {
    webServer.database.mariadb.exec(
        "DELETE FROM `topic`", []() -> void { VLOG(0) << "Delete successful"; },
        [](const std::string &errorString, unsigned int errorNumber) -> void {
          VLOG(0) << "Query failed: " << errorString << " : " << errorNumber;
        });
    ...
    webServer.database.mariadb.exec(
        "DELETE FROM `post`;", []() -> void { VLOG(0) << "Delete successful"; },
        [](const std::string &errorString, unsigned int errorNumber) -> void {
          VLOG(0) << "Query failed: " << errorString << " : " << errorNumber;
        });
