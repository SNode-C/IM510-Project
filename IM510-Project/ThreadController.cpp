...
void ThreadController::initRoutes(WebServer &webServer)
{
    ...
    webServer.webApp.get("/threads", [this, &webServer]APPLICATION(req, res){
        ...
        webServer.database.mariadb.query(
            ...
            [this, &webServer, &res, currentTopicId, loggedInString](const MYSQL_ROW row) -> void  {
                ...
                if (row != nullptr) {
                    ...
                    res.send(
                        ...
                        "<div class=\"buttons\">"
                            ...
                            "</a>"
                            "<a href=\"/createThread?topic__id=" + currentTopicId + "\">"
                                "<button " + buttonDisabledLoggedOff + ">Create Thread</button>"
                            "</a>"
                            + loggedInString +
                        "</div>"
                        "<div class=\"main\">"
    ...
    webServer.webApp.get("/createThread", [this, &webServer]APPLICATION(req, res) {

        std::string topicId = getParam(req, "topic__id");

        webServer.database.mariadb.query(
            "SELECT `id` FROM users WHERE `username` = '" + webServer.getLoggedInUserName() + "'",
            [this, &webServer, &res, topicId](const MYSQL_ROW row) -> void {
                if (row != nullptr) {
                    StyleHtmlProvider styleProvider;
                    ...
                    res.send(
                        ...
                        "<div class=\"main\">"
                            ...
                            "<form method=\"post\" enctype=\"multipart/form-data\">"
                                ...
                                "<input type=\"hidden\" name=\"topic__id\" value=\"" + topicId + "\" />"
    ...
    // Create a new thread
    webServer.webApp.post("/createThread", [this, &webServer]APPLICATION(req, res) {
